...
def data_x_y_for_sgd(x_for_len):
    index_random_for_x_y = random.choice(range(len(x_for_len)))
...
def minimz(data_x_y, theta_0, l_r):
    ...
    while n < 1000:
        ...
        print("\t" * 4 ,step_size_for_a_b, " step_size_ᶐ_β")
        theta_n = theta_0
        print(theta_n, " theta_0 before переопределения")
        theta_0 = [theta_0[k] - sum_grad_a_b[k] * step_size_for_a_b for k in range(2)]  # индекс k - выбор для ᶐ или β
        print(theta_0, "theta_0 after переопределения")

        quotient_no_improvement = [theta_0 == theta_n for _ in range(2)]  # если theta_n не меняется 5 раз - останов
        print(quotient_no_improvement, " quotient_no_improvement")

        data_x_y = [i for i in data_x_y_for_sgd(x_data)] #
        print(data_x_y, ">>>>>>>>>>>>>>>>>>>>>>>>>")

        if all(quotient_no_improvement) == True:

        # if bool ([_ for _ in filter(lambda row: row is False, quotient_no_improvement)] ) == False: # повтор ᶐ и β
...
print(minimz(data_x_y_for_sgd(x_data), theta_0, 0.1), " minimz")
