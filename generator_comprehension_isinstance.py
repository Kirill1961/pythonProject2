...
print((dict_dif), "получаем новый словарь key  из "
...
a = [[1, 2, 3], [4, 5, 6]]
print(list(j for i in a for j in i), "Объединение двойных вложенных списков LIST[LIST] в [LIST]", "\n")
...
print(triple, " Объединение тройных  вложенных списков LIST[LIST[LIST]] в LIST[LIST]", "\n")
...
print([i[0] * i[1] for i in list(zip([2, 5], [3, 10]))], " Почленное умножение списков [2,5], [3,10]", "\n")
print(" или", "\n")
print([i * j for i, j in list(zip([100, 5], [3, 100]))], " Почленное умножение списков [100,5], [3,100]", "\n")

# Запись из первого списка во второй со стиранием первого списка
a = [1, 2]
b = [3, 4]
a.clear(), [a.append(i) for i in b]
print(" Запись из списка b=[3, 4] в список a=[1, 2] со стиранием   списка а;", "a=", a, "b=", b, "\n")

#  Фильтр списка по заданным параметрам, здесь по первому префексу строчного ОБ
a = ["Asdf", "Sddf", "Sdre", "Gerty"]
print([i for i in a if i.startswith("G")], "Фильтр списка по заданным параметрам, "
                                           "здесь по первому префексу строчного ОБ", "\n")
...
print([[p for p, t in zip(b, a) if t == i] for i in range(k)], "K-Means - sort ОБ принадлежащих "
                                                               "кластерам по индексам кластеров", "\n")
...
#  Генерация  из заданного LIST - LIST разной длинны и в случайной последовательности
a = [14, 8, 0, 16, 11, 16, 1, 0, 18, 1, 0, 2, 18, 5, 1, 19, 13, 10, 5, 20, 15, 2, 19, 1, 8]


def generation_list_different_len(chunk_size):
    d = list(map(lambda i: [a[r: (r + i)] for r in range(0, len(a), i)], [rn.randint(2, 5) for i in range(chunk_size)]))
    c = [i for j in d for i in j]
    rn.shuffle(c)
    return c


print(generation_list_different_len(5), "Генерация  из заданного LIST - "
...
print([(i, j) for i in range(5) for j in range(5)], "Создание всех вариантов пар из списка", "\n")
