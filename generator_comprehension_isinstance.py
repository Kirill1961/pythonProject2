...
# TODO Ленивый генератор
#  * Генератор, он не вычисляет и не хранит все значения сразу
#  * Генератор ожидает, пока вы запросите значения, и только тогда вычисляет и возвращает их.
#  * процесс продолжается, пока генератор не исчерпает все значения из range(10)

gen = (x for x in range(10) if x % 2 == 0)

print(next(gen))  # 0 (генератор вычислил и вернул первое значение)
print(next(gen))  # 2 (генератор вычислил и вернул второе значение)
# print(next(gen))  # 4 (генератор вычислил и вернул третье значение)