...
def variance(x_):  # ДИСПЕРСИЯ Или variance
    ...
    print (variance_su_sq,'         ДИСПЕРСИЯ или variance это сумма квадратов deviations / n - 1', "\n" )
...
def covariance(x_, y_):
    ...
    print(x_dev , y_dev, '  deviations - Отклонение от среднего д/ вариационных рядов " х_ " и " у_ "', "\n")
    print([i for i in zip(x_dev , y_dev)], " >>> пары отклонений от среднего Х и У >>>", "\n")
    sum_multiply = [i * k for i, k in zip(x_dev , y_dev)] # произведение разбросов x_dev и y_dev
    return (sum(sum_multiply)/(n - 1)) # covariance - сумма произведений отклонений Х и У от среднего
...
print(outlier_index, " индекс выброса значения сильно отличающегося от всего ряда", "\n")
...
def sum_of_square_error(*alpha_beta):
    return sum([error(*alpha_beta, x_i, y_i, )**2 for x_i, y_i in zip(num_friends_three, daily_minutes) ])


print(sum_of_square_error(27.849969715324047, 0.05784373107207752))  # *alpha_beta - подставляем коэф alpha и beta
                                                                      # , вычисленные МНК
...
def sum_least_squares(alpha, beta, y_mean):
    return sum([(predict(alpha, beta, y_predict) - y_mean)**2 for y_predict in daily_minutes])
...
def r_square(alpha, beta ):
    ...
    print(total_sum_of_squares(mean(daily_minutes)), " total_sum_of_squares")
    return 1 - (sum_least_squares(alpha, beta, mean(daily_minutes)) / total_sum_of_squares(mean(daily_minutes)))
...
def r_r_square(alpha, beta ):
    print(sum_least_squares(alpha, beta, mean(daily_minutes)), " sum_least_squares")
    print(total_sum_of_squares(mean(daily_minutes)), " total_sum_of_squares")
    return 1 - (sum_of_square_error(alpha, beta) / total_sum_of_squares(mean(daily_minutes)))
...
def squared_error(alpha, beta, x_i, y_i):
    ...
    return [error(alpha, beta, i, j) ** 2 for i, j in zip(num_friends_three, daily_minutes)]
