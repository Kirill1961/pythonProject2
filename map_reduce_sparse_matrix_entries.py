...
def matrix_multiply_mapper(m, element):
    for elem in [j for i in element for j in i]:
        name, (i, j), value = elem[0], elem[1], elem[2]  # выделение компонентов из элементов - name, i, j, value
...
def matrix_multiply_reducer(m, entries):
    ...
    for key, index, value in entries:
        ...
        coordinates_values_total = [(r[0: comon_size], r[comon_size:]) for r in result_by_index.values()
        ...
        coordinates = [set([(k[0], v[0]) for k, v in zip(i[0], i[1]) if all((k[1], v[1]))])
                       for i in coordinates_values_total]

        # умножение и сложение величин двух матриц;
        # if i[k] - удаляет пустые set(),  False
        values_not_zero = [sum([(k[1] * v[1]) for k, v in zip(pair[0], pair[1])]) for pair in coordinates_values_total]

        # соединение вычисленных значений с соответствующими им координатами; if pair[0] - удаляет пустые set(),  False
        result_multiply_spars_matrix = [pair for pair in zip(coordinates, values_not_zero) if pair[0]]
