...
# Условная индексация
a = np.arange(1, 11)
print(a[a % 2 == 0], "Условная индексация - вывод чётных значений из массива", "\n")
...
a = np.round(a, 2)
print(a, "\n")
print(a[1, 1], 'Последняя строка последний столбец', "\n")

print('Двоеточие добавляет размерность')
print(a[1:, 1])
print(a[1:, 1:], "\n")

print(a[1, 0:], 'Последняя строка и все столбы',  "\n")
print(a[:, 0], 'Все строки и первый столбец', "\n")

# TODO Треугольные мытрицы
print('Треугольная нижняя матрица :', '\n', f'{np.tril(np.ones(9).reshape(3, 3))}', '\n')
print('Треугольная верхняя матрица :', '\n', f'{np.triu(np.ones(9).reshape(3, 3))}', '\n')


# TODO Единичная и Нулевая матрица по размеру заданной матрицы для Маски
#  ones_like  и zeros_like - Маски для заданных матриц
ar = np.arange(1, 10).reshape(3, 3)
print('Единичная и Нулевая матрица по размеру заданной матрицы 3x3:', '\n', f'{np.ones_like(ar)}')
