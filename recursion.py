...
def foo(ls):
    if ls == []:
        return 0
    return foo(ls[1:]) + ls[0]


print('Сумма с  list в одну строку : \n', foo([2, 4, 7, 1]))
...
def foo(ar):
    ...
    less = [
        x for x in ar[1:] if x < pivot
    ]  # Ркурсивный случай, Уменьшение аргумента ar, массив в слева от опорного элемента
    greater = [
        x for x in ar[1:] if x > pivot
    ]  # Ркурсивный случай, Уменьшение аргумента ar, но массив в справа от опорного элемента
    return (
        foo(less) + [pivot] + foo(greater)
    )  # Сложение списков в отсортированный список
