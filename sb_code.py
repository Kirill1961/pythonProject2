...
def A_B_rate_restore(a_rate, b_rate, window, sigma):
    ...
    В окне считаем статистики, затем основываясь на среднеквадратичном отклонении от среднего,
    которое по сути является std определяем аутлаеры
    ...
    * 2 суток устойчивого направления
    ...
    for n in range(1, len(a_rate)):
        ...
        # 👉 result_a[-20:] это скользящее окно для заполняемого списка result_a, 20 - ткущий уровень (среднее)
        # 👉 окно всегда в прошлом, позади текущего значения
        # 👉 Среднее последних 20 наблюдений — оценка текущего среднего уровня ряда
        ...
        # 👉 текущий уровень (среднее) + средняя скорость изменения
        # 🚀 текущий уровень (среднее) - точка вокруг которой происходит колебания тренда и сезонности
...
# 👉 min_periods=1, среднее вычисляется начигная с 1-й строки, что бы не было NA в начале таблицы
...
__doc__ = '''
Адаптивный лаг:
* расход вырос → материал движется быстрее → лаг уменьшается;
* расход упал → материал движется медленнее → лаг увеличивается.
67 — это средний или номинальный расход B_rate, вычисленный на куске данных, на котором нашли оптимальный лаг 198.
1 CV нашёл лучший лаг = 198;
2 вычислили средний расход = 67;
3 дальше лаг динамически пересчитывается по текущему расходу;
4 признаки берутся не из фиксированного i-198, а из i-shift.
'''
...
for i in range(features.shape[0]):
    if i < 250:
        ...
        # был посчитан средний rate и оптимальный сдвиг на небольшом куске данных и потом в цикле значения уточнялись
        shift = 67 / b_rate * 198  # 👉 Сдвиг в зависимости от расхода в точке В
...
# 👉 Индексы новых ненулевых значений, определяем через Тильду как ненулевые значения A_rate_y
idx = comb_data[~comb_data['A_rate_y'].isnull()].index
# Копируем первые 10 столбцов
comb_train = final_train.iloc[:, :10].copy()
# заменяем часть трейна через индексы ненулевых строк на новые данные
...
# финальный тест это последние 3984 строки - [-3984:]
...
# cv - подготовка 16 обучающих окон для построения ансамбля прогнозов на всём датасете
...
for num, target in enumerate(final_targets.columns):
    ...
    # except:
        # Ассамблирование - предсказываем значения для валидации
