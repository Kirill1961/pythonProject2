...
# TODO  `walrus` - результат движется `"туда - обратно"`:
#  - одновременно `присваивает значение` переменной и `участвует в выражении`.
...
# Многоступенчатое использование оператора := в списковом включении
